...
async def private(bot, message):
    ...
    if not _user:
        return await msg_edit(m, "<b>You didn't added any bot. Please add a bot using /settings !</b>", wait=True)
    try:
        client = await start_clone_bot(CLIENT.client(_user))
    except Exception as e:
        return await m.edit(e)
    await msg_edit(m, "<b>processing..</b>")
    try:
        ...
        await msg_edit(m, f"**Source chat may be a private channel / group. Use userbot (user must be member over there) or  if Make Your [Bot](t.me/{_user['username']}) an admin over there**", retry_btn(frwd_id), True)
        return await stop(client, user)
    try:
        k = await client.send_message(i.TO, "Testing")
        await k.delete()
    except:
        await msg_edit(m, f"**Please Make Your [UserBot / Bot](t.me/{_user['username']}) Admin In Target Channel With Full Permissions**", retry_btn_private(frwd_id), True)
        return await stop(client, user)
    temp.forwardings += 1
    await db.add_frwd(user)
    await send(client, user, "<b>🧡 ғᴏʀᴡᴀʀᴅɪɴɢ sᴛᴀʀᴛᴇᴅ</b>")
    sts.add(time=True)
    sleep = 1 if _user['is_bot'] else 10
    await msg_edit(m, "<b>Processing...</b>")
    temp.IS_FRWD_CHAT.append(i.TO)
    temp.lock[user] = locked = True
    if locked:
...
async def edit(msg, title, status, sts):
    i = sts.get(full=True)
    status = 'Forwarding' if status == 10 else f"Sleeping {status} s" if str(
        status).isnumeric() else status
    percentage = "{:.0f}".format(math.floor(
    ...
    time_to_completion = round(sts.divide(
        i.total - i.fetched, int(speed))) * 1000
    estimated_total_time = elapsed_time + time_to_completion
    progress = "{0}{1}".format(
        ''.join(["▰" for i in range(math.floor(int(percentage) / 10))]),
        ''.join(["▱" for i in range(10 - math.floor(int(percentage) / 10))]))
    button = [[InlineKeyboardButton(
        progress, f'fwrdstatus#{status}#{estimated_total_time}#{percentage}#{i.id}')]]
    ...
    if status in ["cancelled", "completed"]:
        button.append([InlineKeyboardButton('💟 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ 💟', url='https://t.me/+mCdsJ7mjeBEyZWQ1')])
        button.append([InlineKeyboardButton('💠 ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ 💠', url='https://t.me/Kdramaland')])
    else:
...
def retry_btn_private(id):
    return InlineKeyboardMarkup([[InlineKeyboardButton('⟲ RETRY', f"start_private_{id}")]])
...
async def status_msg(bot, msg):
    ...
    est_time = est_time if (est_time != '' or status not in [
                            'completed', 'cancelled']) else '0 s'
