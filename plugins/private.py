...
@Client.on_message(filters.private & filters.command(["private_forward"]))
async def run(bot, message):
    ...
    if not _user:
      return await message.reply("<b>You didn't added any user. Please add a bot using /settings !</b>")
    ...
    if len(channels) > 1:
       ...
       _toid = await bot.ask(message.chat.id, Translation.TO_MSG.format(_user['name'], _user['username']), reply_markup=ReplyKeyboardMarkup(buttons, one_time_keyboard=True, resize_keyboard=True))
  ...
  #  except ChannelInvalid:
    ...
    buttons = [[
        InlineKeyboardButton('Yes', callback_data=f"start_private_{forward_id}"),
        InlineKeyboardButton('No', callback_data="close_btn")
    ]]
    reply_markup = InlineKeyboardMarkup(buttons)
    await message.reply_text(
        text=Translation.DOUBLE_CHECK.format(botname=_user['name'], botuname=_user['username'], from_chat=title, to_chat=to_title, skip=skipno.text),
