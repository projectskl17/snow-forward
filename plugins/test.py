...
from typing import Union, Optional, AsyncGenerator, Any, Dict
    ...
    async def iter_messages(
        ...
        while True:
            ...
            for message in messages:
                if get_message_types(message=message) not in filters:
                    yield message
                    current += 1
    #
...
async def update_configs(user_id, key, value):
    ...
    if key in ['batch', 'delete_time', 'caption', 'duplicate', 'db_uri', 'forward_tag', 'protect', 'file_size', 'size_limit', 'extension', 'keywords', 'button']:
        current[key] = value
    else:
