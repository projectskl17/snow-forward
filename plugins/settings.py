...
async def settings_query(bot, query):
    ...
    async def get_bot_and_user():
        _bot = await db.get_bot(user_id)
        _user = await db.get_user(user_id)
        return _bot, _user
    ...
    if type == "main":
        ...
        buttons = []
        _bot, _user = await get_bot_and_user()

        if _bot is not None:
            buttons.append([InlineKeyboardButton(
                _bot['name'], callback_data="settings#editbot")])

        if _user is not None:
            buttons.append([InlineKeyboardButton(
                _user['name'], callback_data="settings#edituser")])
        else:
        ...
        bot = await db.get_user(user_id)
        TEXT = Translation.BOT_DETAILS if bot['is_bot'] else Translation.USER_DETAILS
        buttons = [[InlineKeyboardButton('❌ Remove ❌', callback_data=f"settings#removeuser")
                    ],
                   [InlineKeyboardButton('⟪ back', callback_data="settings#bots")]]
        await query.message.edit_text(
            TEXT.format(bot['name'], bot['id'], bot['username']),
    ...
    elif type == "extrafilters":
        await query.edit_message_reply_markup(
            reply_markup=await extra_filters_buttons(user_id))
        ...
        btn.append([InlineKeyboardButton(
            'Remove all', 'settings#rmve_all_extension')])
        ...
        btn.append([InlineKeyboardButton(
            'Remove all', 'settings#rmve_all_keyword')])
        btn.append([InlineKeyboardButton('⟪ back', 'settings#main')])
        await query.message.edit_text(
...
def main_buttons():
    buttons = [[
        ...
        InlineKeyboardButton('🗃️ ᴅᴀᴛᴀʙᴀsᴇ',
                             callback_data=f'settings#database')
    ], [
        InlineKeyboardButton('🌦️ ғɪʟᴛᴇʀs',
        ...
        InlineKeyboardButton('✨ ᴇxᴛʀᴀ ғᴇᴀᴛᴜʀᴇ',
                             callback_data=f'settings#extrafilters')
    ]]
...
def extract_btn(datas):
    ...
    if datas:
        for data in datas:
            ...
            if i == 0:
                btn.append([InlineKeyboardButton(
                    data, f'settings#alert_{data}')])
                i += 1
                continue
            elif i > 0:
                btn[-1].append(InlineKeyboardButton(data,
                               f'settings#alert_{data}'))
                i += 1
    return btn
...
async def extra_filters_buttons(user_id):
    filter = await get_configs(user_id)
    filters = filter['filters']
    buttons = [[
