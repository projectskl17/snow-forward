...
class Database:
    ...
    async def add_bot(self, bot_datas):
        if not await self.is_bot_exist(bot_datas['user_id']):
            await self.bot.insert_one(bot_datas)

    async def add_bot_user(self, user_datas):
    ...
    async def remove_bot(self, user_id):
        await self.bot.delete_many({'user_id': int(user_id), 'is_bot': True})

    async def remove_user(self, user_id):
        await self.bot.delete_many({'user_id': int(user_id), 'is_bot': False})

    async def get_bot(self, user_id: int):
        bot = await self.bot.find_one({'user_id': user_id, 'is_bot': True})
